    ...
    return {
        ...
        visitor: {
            ClassDeclaration(nodePath) {
                ...
                if (!classDecoratorNodes) { // filter #1, no decorators for current class
                    return;
                }
                const cppClasses = getExportedClassesFromCppSourceCode();
                if (!cppClasses.has(currentClassName)) { // filter #2, no C++ binding type found with the same name
                    return;
                }
                ...
                // filter #3, return if no ccclass found or classes from `sp.` (spine) modules
        ...
        fs.readFileSync(file, 'utf8').split('\n').map((x) => x.trim()).filter((x) => {
            ...
            if (createClassR) {
                ...
                if (pathComps.includes('dragonbone') || pathComps.includes('spine')) {
                    // filter #4, skip spine/dragonbones binding source files
                    return null;
                }
